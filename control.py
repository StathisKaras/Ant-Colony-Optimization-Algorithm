...
class Control():
    ...
    def f(var):
        ...
        x, y, z, w = var
        return -20 * np.exp(-0.2 * np.sqrt(0.25 * (x ** 2 + y ** 2 + z ** 2 + w ** 2))) - np.exp(0.25 * (np.cos(2 * np.pi * x) + np.cos(2 * np.pi * y) + np.cos(2 * np.pi * z) + np.cos(2 * np.pi * w))) + 20 + np.e +100

    # Minimum value: 100 at (0, 0, 0, 0)
    # Ackley function
    # Domain: x, y, z, w ∈ [-5, 5]
